...
variable "use_existing_subnets" {
  type        = bool
  description = "Whether the existing subnet is used for the network resources"
}

variable "network_params" {
  description = "A map of networkk parameters for load balancer, bastion, fss, admin server instance, managed server instance, and database"
  type        = object({
    wls_subnet_id                  = string
    bastion_subnet_id              = string
    bastion_ip                     = string
    lb_subnet_1_id                 = string
    lb_subnet_2_id                 = string
    ...
    wls_ms_extern_port             = number
    existing_admin_server_nsg_id   = string
    existing_managed_server_nsg_id = string
    existing_lb_nsg_id             = string
    existing_mount_target_nsg_id   = string
    existing_bastion_nsg_id        = string
    lb_source_cidr                 = string
  })
} 
