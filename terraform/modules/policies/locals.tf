...
locals {
  ...
  autoscaling_statement4  = var.use_autoscaling ? "Allow dynamic-group ${oci_identity_dynamic_group.wlsc_functions_principal_group[0].name} to inspect tenancies in tenancy" : ""
  autoscaling_statement5  = var.use_autoscaling ? "Allow dynamic-group ${oci_identity_dynamic_group.wlsc_functions_principal_group[0].name} to inspect limits in tenancy" : ""
  autoscaling_statement6  = var.use_autoscaling ? "Allow dynamic-group ${oci_identity_dynamic_group.wlsc_functions_principal_group[0].name} to manage volume-family in compartment id ${var.compartment_id}" : ""
  autoscaling_statement7  = var.use_autoscaling ? "Allow dynamic-group ${oci_identity_dynamic_group.wlsc_functions_principal_group[0].name} to manage instance-family in compartment id ${var.compartment_id}" : ""
  autoscaling_statement8  = var.use_autoscaling ? "Allow dynamic-group ${oci_identity_dynamic_group.wlsc_functions_principal_group[0].name} to use app-catalog-listing in compartment id ${var.compartment_id}" : ""
  autoscaling_statement9  = var.use_autoscaling ? "Allow dynamic-group ${oci_identity_dynamic_group.wlsc_functions_principal_group[0].name} to manage virtual-network-family in compartment id ${var.network_compartment_id}" : ""
  autoscaling_statement10 = var.use_autoscaling ? "Allow dynamic-group ${oci_identity_dynamic_group.wlsc_functions_principal_group[0].name} to manage load-balancers in compartment id ${var.network_compartment_id}" : ""
  autoscaling_statement11 = var.use_autoscaling ? "Allow dynamic-group ${oci_identity_dynamic_group.wlsc_functions_principal_group[0].name} to manage orm-family in compartment id ${var.compartment_id}" : ""
  autoscaling_statement12 = var.use_autoscaling ? "Allow dynamic-group ${oci_identity_dynamic_group.wlsc_functions_principal_group[0].name} to read metrics in compartment id ${var.compartment_id}" : ""
  ...
  autoscaling_statement14 = var.use_autoscaling ? "Allow dynamic-group ${oci_identity_dynamic_group.wlsc_functions_principal_group[0].name} to manage repos in tenancy" : ""
  autoscaling_statement15 = var.use_autoscaling ? "Allow dynamic-group ${oci_identity_dynamic_group.wlsc_functions_principal_group[0].name} to manage functions-family in compartment id ${var.compartment_id}" : ""
  autoscaling_statement16 = var.use_autoscaling ? "Allow dynamic-group ${oci_identity_dynamic_group.wlsc_functions_principal_group[0].name} to manage ons-topics in compartment id ${var.compartment_id}" : ""
  ...
  autoscaling_statement19 = var.use_autoscaling ? "Allow dynamic-group ${oci_identity_dynamic_group.wlsc_functions_principal_group[0].name} to manage instance-agent-command-family in compartment id ${var.compartment_id}" : ""
  autoscaling_statement20 = var.use_autoscaling ? "Allow dynamic-group ${oci_identity_dynamic_group.wlsc_functions_principal_group[0].name} to use apm-domains in compartment id ${var.apm_domain_compartment_id}" : ""
  autoscaling_statement21 = var.use_autoscaling ? "Allow dynamic-group ${oci_identity_dynamic_group.wlsc_functions_principal_group[0].name} to manage alarms in compartment id ${var.compartment_id}" : ""
  autoscaling_statement22 = var.use_autoscaling ? "Allow dynamic-group ${oci_identity_dynamic_group.wlsc_functions_principal_group[0].name} to manage log-groups in compartment id ${var.compartment_id}" : ""
  autoscaling_statement23 = var.use_autoscaling ? "Allow dynamic-group ${oci_identity_dynamic_group.wlsc_functions_principal_group[0].name} to use unified-configuration in compartment id ${var.compartment_id}" : ""
  ...
  autoscaling_statement27 = var.use_autoscaling ? "Allow dynamic-group ${oci_identity_dynamic_group.wlsc_functions_principal_group[0].name} to inspect dynamic-groups in tenancy" : ""
  autoscaling_statement28 = var.use_autoscaling ? "Allow dynamic-group ${oci_identity_dynamic_group.wlsc_functions_principal_group[0].name} to manage policies in tenancy" : ""
  autoscaling_statement29 = var.use_autoscaling ? "Allow dynamic-group ${oci_identity_dynamic_group.wlsc_functions_principal_group[0].name} to use tag-namespaces in tenancy" : ""
  autoscaling_atp_policy_statement     = (var.atp_db.is_atp && var.use_autoscaling) ? "Allow dynamic-group ${oci_identity_dynamic_group.wlsc_functions_principal_group[0].name} to inspect autonomous-transaction-processing-family in compartment id ${var.atp_db.compartment_id}" : ""
  autoscaling_db_policy_statement     = (var.oci_db.is_oci_db && var.use_autoscaling) ? "Allow dynamic-group ${oci_identity_dynamic_group.wlsc_functions_principal_group[0].name} to inspect database-family in compartment id ${var.oci_db.compartment_id}" : ""
  autoscaling_fss_mount_target_policy_statement = (var.add_fss && var.use_autoscaling) ? "Allow dynamic-group ${oci_identity_dynamic_group.wlsc_functions_principal_group[0].name} to manage mount-targets in compartment id ${var.fss_compartment_id}" : ""
  autoscaling_fss_file_system_policy_statement = (var.add_fss && var.use_autoscaling) ? "Allow dynamic-group ${oci_identity_dynamic_group.wlsc_functions_principal_group[0].name} to manage file-systems in compartment id ${var.fss_compartment_id}" : ""
  autoscaling_fss_export_sets_policy_statement = (var.add_fss && var.use_autoscaling) ? "Allow dynamic-group ${oci_identity_dynamic_group.wlsc_functions_principal_group[0].name} to manage export-sets in compartment id ${var.fss_compartment_id}" : ""
