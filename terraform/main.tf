...
module "provisioners" {
  ...
  assign_public_ip                 = local.assign_weblogic_public_ip
  bastion_host                     = var.is_rms_private_endpoint_required ? "" : (local.assign_weblogic_public_ip || !local.is_bastion_instance_required ? "" : var.existing_bastion_instance_id == "" ? module.bastion[0].public_ip : data.oci_core_instance.existing_bastion_instance[0].public_ip)
  bastion_host_private_key         = var.is_rms_private_endpoint_required ? "" : (local.assign_weblogic_public_ip || !local.is_bastion_instance_required ? "" : var.existing_bastion_instance_id == "" ? module.bastion[0].bastion_private_ssh_key : file(var.bastion_ssh_private_key))
  is_bastion_instance_required     = local.is_bastion_instance_required
