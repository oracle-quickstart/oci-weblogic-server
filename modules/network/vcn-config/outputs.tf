...
output "wls_security_list_id" {
  ...
  value       = compact(concat(oci_core_security_list.wls_security_list.*.id, list("")))
}

output "wls_internal_security_list_id" {
  description = "OCID of security list for WLS public subnet. "
  value       = compact(concat(oci_core_security_list.wls_internal_security_list.*.id, list("")))
}

output "wls_ms_security_list_id" {
  description = "OCID of security list for WLS or bastion subnet. "
  value       = compact(concat(oci_core_security_list.wls_ms_security_list.*.id, list("")))
}

output "lb_security_list_id" {
  description = "OCID of security list for LB subnet. "
  value       = compact(concat(oci_core_security_list.lb_security_list.*.id, list("")))
}
...
output "fss_security_list_id" {
  ...
  value       = compact(concat(oci_core_security_list.fss_security_list.*.id, list("")))
}
...
output "route_table_id" {
  ...
  value       = concat(coalescelist(oci_core_default_route_table.wls_default_route_table1.*.id, oci_core_route_table.wls_route_table2.*.id, list("")), list(""))
}

output "service_gateway_route_table_id" {
  description = "OCID of route table with service gateway. "
  value       = element(coalescelist(oci_core_route_table.wls_gateway_route_table_newvcn.*.id, oci_core_route_table.wls_gateway_route_table_existingvcn.*.id, list("")), 0)
}

/* DHCP OPTIONS */
output "dhcp_options_id" {
  description = "OCID of DHCP options. "
  value       = element(coalescelist(oci_core_dhcp_options.wls_dhcp_options1.*.id, list("")), 0)
}
